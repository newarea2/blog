...
export const getDateList = (startDate, endDate) => {
  ...
  const dateList = []
  // 不能直接 dateList.push(startDate)，new Date(startDate) 的作用相当于克隆开始时间
  dateList.push(new Date(startDate))
  while (startDate.getTime() < endDate.getTime()) {
    startDate.setDate(startDate.getDate() + 1)
    /**
     * 这里如果写成 dateList.push(startDate))，得到错误的结果，结果中第一项是第一天，后面所有的都是最后一天
     * 这里 new Date(startDate) 的作用相当于 JSON.parse(JSON.stringify(foo))
     */
    dateList.push(new Date(startDate))
  }
  return dateList
 ...
 * formatDuration1(new Date('2018-11-23') - new Date('2018-11-20')) // '72小时0分钟0秒'
 */
export const formatDuration1 = (time) => {
  time = time / 1000 // 单位秒
 ...
 * formatDuration2(new Date('2018-11-20') - new Date('2018-11-23')) // '已超时72小时0分钟0秒'
 */
export const formatDuration2 = (time) => {
 ...
 * @param {String} d2 
 * @example
 * formatDuration('2019-01-25 08:00:10', '2019-01-25 08:00:00') // 10秒
 * formatDuration('2019-01-25 08:01:10', '2019-01-25 08:00:00') // 1分钟10秒
 * formatDuration('2019-01-25 09:01:10', '2019-01-25 08:00:00') // 1小时1分钟10秒
 */
export const formatDuration3 = (d1, d2) => {
  let time = new Date(d1).valueOf() - new Date(d2).valueOf()
  time = time / 1000
  const hour = parseInt(String(time / 3600))
  const min = parseInt(String((time % 3600) / 60))
  const sec = parseInt(String((time % 3600) % 60))
  if (!hour && !min) return `${sec}秒`
  if (!hour) return `${min}分钟${sec}秒`
  return `${hour}小时${min}分钟${sec}秒`
}
 ...
 * @param {*} endDate 
 * @example
 * formatDuration4('2023-05-20 00:00:00', '2023-05-25 15:11:50') // 5天15小时11分50秒
 */
export const formatDuration4 = (startDate, endDate) => {
  startDate = new Date(startDate)
  endDate = new Date(endDate)
